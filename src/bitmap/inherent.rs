...
impl RoaringBitmap {
    ...
    pub fn push_unchecked(&mut self, value: u32) {
        let (key, index) = util::split(value);
    ...
    ///
    /// If debug_assertions enabled and index is > self.max()
    pub fn push_unchecked_prealloc(&mut self, value: u32) {
        let (key, index) = util::split(value);
        ...
        match self.containers.last_mut() {
            ...
            _otherwise => {
                let mut container = Container::new_with_array_limit_capacity(key);
                container.push_unchecked(index);
                self.containers.push(container);
            }
        }
